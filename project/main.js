      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          // 5.- List by name all companies founded in the summer of 2004 (april to june) sorted by date.
            db.collection('companies').find({ $and: [{ founded_year: 2004 }, { $or: [{ founded_month: 4 }, { founded_month: 5 }, { founded_month: 6 }] }] }, { name: 1, _id: 0 }).sort({ founded_month: 1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => {
                mainMenu()
