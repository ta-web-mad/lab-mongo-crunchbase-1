        ...
        switch(option){
          ...
          case "5":
            //List by name all companies founded in the summer of 2004 (april to june) sorted by date.
              db.collection('companies').find({ $and: [{ founded_year: 2004 } , { founded_month: 4}, { founded_month: 5}, { founded_month: 6}] }, {name: 1, _id: 0}, {founded_month: 1}).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          case "9":
              ...
              db.collection('companies').find({ $and: [{ name: "Facebook" }, { number_of_employees: { $gt: 0 } }] }, {name: 1, _id: 0}).count((error, result) => {
                if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "10":
              ...
              db.collection('companies').find({ $and: [{ name: "Facebook" }, { products: { $all: Object } }] }, {name: 1, _id: 0}).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "11":
              ...
              db.collection('companies').find({ $and: [{ name: "Facebook" }, { relationships: { is_past: true } }] }, {name: 1, _id: 0}).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "12":
            ...
            db.collection('companies').find({ $and: [{ name: "Facebook" }, { relationships: { is_past: false } }] }, {name: 1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "14":
            //List by name the competitors of Facebook
              db.collection('companies').find({ $and: [{ name: "Facebook" }, { competitions:  { $all: [name] } }] }, {name: 1, _id: 0}).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          case "15":
            ...
            case "16":
              //How many companies that has "social-network" in tag-list and founded between 2002 and 2016 inclusive
                db.collection('companies').find({ $and: [{ year: { $gt: 2001 } }, { year: { $lt: 2017 } }] } , {tag_list: "social-network"}, {name: 1, _id: 0}).count((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                  } else {
            ...
            case "17":
              //Names and locations of companies that have offices in London
                db.collection('companies').find({ $and: [{ name: "" }, { "offices.city":  { $all: ["London"] } }] }, {name: 1, _id: 0}).count((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                  } else {
