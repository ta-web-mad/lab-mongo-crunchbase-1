      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "3":
            ...
            // Se corrigió el ejercicio 2 porque funded es obtener fondos (ruedas de inversión) y founded es de fundar.
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ 'founded_month': { $gte: 4 } }, { 'founded_month': { $lte: 6 } }, { 'founded_year': 2004 },] }, { name: 1, _id: 0 }).sort({ 'founded_month': 1, 'founded_year': 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "6":
            ...
            db.collection('companies').find({ "offices.city": { $all: ["Barcelona"] } }, { name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
