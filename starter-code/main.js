      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "11":
            ...
            db.collection('companies').find({ $and: [{ name: "Facebook", "relationships.is_past": { $eq: true } }] }, { "relationships": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "13":
            ...
            db.collection('companies').find({ name: "Facebook" }, { "competitions.competitor.name": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                ...
                const [arr] = result.map(e => e.competitions);

                //console.log(arr);

                arr.map(e => console.log(e.competitor.name));
          ...
          case "17":
            ...
            db.collection('companies').find({ $and: [{ tag_list: "social-networking" }, { "offices.city": "New York" }, { founded_year: { $gte: 2002 } }, { founded_year: { $lte: 2016 } }] }, { name: 1, _id: 0, "offices.city": 1 }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
