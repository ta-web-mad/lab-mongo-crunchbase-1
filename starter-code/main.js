      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "10":
            ...
            db.collection('companies').find({ name: "Facebook" }, { products: { $elemMatch: { name: { $exists: true } } } }, { name:1,  _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "11":
            ...
            db.collection('companies').find({ $and: [{ name: "Facebook" }, { "relationships.is_past": false }] }, { "relationships.person.permalink": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "15":
            ...
            db.collection('companies').find({ $and: [{ tag_list: { $regex: "social-networking" } }, { $and: [{ founded_year: { $gt: 2001 } }, { founded_year: { $lt: 2017 } }] }] }, { name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "17":16
            ...
            db.collection('companies').find({ $and: [{ $and: [{ tag_list: { $regex: "social-networking" } }, { $and: [{ founded_year: { $gt: 2001 } }, { founded_year: { $lt: 2017 } }] }] }, { "offices.city": "New York" }] }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
