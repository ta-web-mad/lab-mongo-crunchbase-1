      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ founded_year: 2004 }, { founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }] }, { _id: 0, name: 1, founded_month: 1, founded_day: 1 }).sort({ founded_month: 1, founded_day: 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "8":
            ...
            db.collection('companies').find({ name: "Facebook" }, { name: 1, category_code: 1, description: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
