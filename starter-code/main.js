			...
			rl.question('Type an option: ', (option) => {
				switch (option) {
					...
					case "5":
						...
						db.collection('companies').find({ $and: [{ founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }, { founded_year: 2004 }] }, { name: 1, _id: 0 }, { founded_year: 1 }).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "11":
						...
						// This does not work. When I filter by the name, it shows every relationship of the company, and I do not know why
						db.collection('companies').find({ $and: [{ "relationships": { $elemMatch: { is_past: { $eq: false } } } }] }).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "12":
						...
						db.collection('companies').find({ $and: [{ "relationships": { $elemMatch: { "person.permalink": "david-ebersman" } } }] }, { name: 1, _id: 0 }).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "15":
						// 15.- How many companies that has "social-network" in tag-list
						// and founded between 2002 and 2016 inclusive
						db.collection('companies').find({ $and: [{ tag_list: /social-networking/i }, { founded_year: { $gte: 2002 } }, { founded_year: { $lte: 2016 } }] }).count((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "17":
						...
						db.collection('companies').find({ $and: [{ tag_list: /social-networking/i }, { founded_year: { $gte: 2002 } }, { founded_year: { $lte: 2016 } }, { offices: { $elemMatch: { city: "New York" } } }] }).count((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
