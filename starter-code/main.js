      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "3":
            ...
            db.collection('companies').find({ $and: [{ "relationships.person.permalink": "david-ebersman" }, {"relationships.is_past": true}]}, { "relationships.person": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ name: "Facebook" }, {"competitions.competitor" : 1 }).sort({"competitions.competitor.name" : 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({"offices.city": "London"}, { name: 1 , "offices.city": 1}).sort({"competitions.competitor.name" : 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
