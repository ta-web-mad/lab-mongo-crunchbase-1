      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "7":
            ...
            db.collection('companies').find({ "number_of_employees": { $gte: 186000 } }, { name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "10":
            ...
            db.collection('companies').find({ "name": "Facebook" }, { _id: 0, products: 1 }).toArray((error, result) => {
              if (error) {
                ...
                let products
                result.forEach(elm => { products = elm })
              ...
              if (error) {
                ...
                result[0].relationships.forEach(elm => {
                  if (elm.is_past === false) {
                    let name = elm.person.first_name + " " + elm.person.last_name
                    names.push(name)
                  }
                })
          ...
          case "18":
            ...
            db.collection('companies').find({ $and: [{ "tag_list": "social-network" }, { "founded_year": { $gte: 2002 } }, { "founded_year": { $lte: 2016 } }, { "offices.city": "New York" }] }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
