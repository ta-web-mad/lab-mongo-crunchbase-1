      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "6":
            ...
            db.collection('companies').find({"offices.city": "Pleasanton"}, { name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            case "11":
              ...
              db.collection('companies').find({"name": "Facebook"}, { name: 1, relationships: 1, _id: 0 }).toArray((error, result) => {
                if (error) {
                  ...
                  let list = result[0].relationships.map( elem => 
                    elem.person.first_name + ' ' + elem.person.last_name)
                  console.log(list);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                }
              })
            ...
            case "13":
              ...
              db.collection('companies').find({"name": "Facebook"}, { name: 1, competitions: 1, _id: 0 }).toArray((error, result) => {
                if (error) {
                  ...
                  result = result[0].competitions.map( elem => elem.competitor.name)
                  // result = [...result]
