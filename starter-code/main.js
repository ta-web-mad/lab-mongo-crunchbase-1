      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "4":
            ...
            db.collection('companies').find({ "founded_month": 2 }, { name: 1, _id: 0 }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ "founded_month": { $gt: 3 } }, { "founded_month": { $lt: 7 } }, { "founded_year": 2004 }] }, { name: 1, _id: 0 }).toArray({ founded_year: 1, founded_month: 1, founded_day: 1 }).sort((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "7":
            ...
            db.collection('companies').find({ name: 1, number_of_employees: 1 }).limit(10), { number_of_employees: -1 }((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "13":
            ...
            db.collection('companies').find({ name: "Facebook" }, { "competitions.competitor": 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "16":
            ...
            db.collection('companies').find({ "offices.city": "London" }, { name: 1, "offices.address1": 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "17":
            ...
            db.collection('companies').find({ tag_list: "social-network" }, { $and: [{ year: { $gt: '2001' } }, { year: { $lt: '2017' } }] }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
