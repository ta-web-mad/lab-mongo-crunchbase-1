      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ founded_month: { $gt: 3 } }, { founded_month: { $lt: 6 } }], founded_year: 2004 }, { founded_month: 1, founded_day: 1, name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "7":
            ...
            db.collection('companies').find({}, { relationships: -1, name: 1, "relationships.person": 1, _id: 0 }).limit(10).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "11":
            ...
            db.collection('companies').find({ name: 'Facebook', "relationships.is_past": false }, { "relationships.person": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
