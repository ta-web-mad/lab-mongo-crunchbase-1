			...
			rl.question('Type an option: ', (option) => {
				switch (option) {
					...
					case '5':
						db.collection('companies')
							.find(
								{ $and: [{ founded_year: 2004 }, { $and: [{ founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }] }] },
								{ name: 1, _id: 0 }
							)
							.toArray((error, result) => {
								if (error) {
					...
					case '7':
						db.collection('companies')
							.find(
								{ $and: [{ number_of_employees: { $exists: true } }, { number_of_employees: { $type: 'number' } }] },
					...
					case '11':
						db.collection('companies')
							.find(
								...
								{ 'relationships.person.first_name': 1, _id: 0 }
							)
							.toArray((error, result) => {
								if (error) {
									console.log(error);
					...
					case '15':
						db.collection('companies')
							.find(
								{ $and: [{ tag_list: /social\-networking/ }, { founded_year: { $gte: 2002 } }, { founded_year: { $lte: 2016 } }] },
								{ _id: 0 }
							)
							.count((error, result) => {
								if (error) {
					...
					case '16':
						db.collection('companies')
							.find({ 'offices.city': 'London' }, { name: 1, 'offices.country_code': 1, _id: 0 })
							.toArray((error, result) => {
								if (error) {
									console.log(error);
									rl.question(`\nType enter to continue: `, (answer) => {
					...
					default:
						rl.question(`\nNot a valid option. Type enter to continue: `, (answer) => {
							mainMenu();
						});
				}
			});
