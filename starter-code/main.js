      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ founded_year: 2004 }, { founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }] }, { name: 1, founded_year: 1, founded_month: 1, founded_day: 1, _id: 0 }).sort(
              { founded_year: 1, founded_month: 1, founded_day: 1 }).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          case "7":
            ...
            db.collection('companies').find({}, { more_employees: 1, name: 1, _id: 0 }).sort({ more_employees: -1 }).limit(10).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "11":
            ...
            // ***** NO SUPE COMO ACCEDER AL OBJETO*****
