      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "4":
            ...
            db.collection('companies').find({ founded_year: [{ founded_year: 2004 }]}, { name: 1, _id: 0 })
          ...
          case "5":
            ...
            db.collection('companies').find({ founded_year: 2004, founded_month: { $gte: 4, $lte: 6 } }, { name: 1, founded_year: 1, founded_month: 1, _id: 0  }).sort({founded_month: 1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
