      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "10":
            ...
            //faltaría quitar las llaves de "products"
            db.collection('companies').aggregate([{ $match: { "name": "Facebook" } }, { $unwind: "$products" }, { $project: { products: "$products.name", _id: 0 } }]).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "11":
            ...
            db.collection('companies').aggregate([{ $match: { name: "Facebook" } }, { $unwind: "$relationships" }, { $match: { "relationships.is_past": { $nin: [true] } } }, { $project: { relations: "$relationships.person.permalink", _id: 0 } }]).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "13":
            ...
            db.collection('companies').aggregate([{ $match: { name: "Facebook" } }, { $unwind: "$competitions" }, { $project: { competitors: "$competitions.competitor.name", _id: 0 } }]).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "14":
            ...
            db.collection('companies').find({ "tag_list": /social-networking$/i }, { name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "15":
            ...
            // si se refiere a que tambien es valido networking, hay que quitar el $
            db.collection('companies').find({ tag_list: /social-network$/i, founded_year: { $gte: 2002 }, founded_year: { $lte: 2016 } }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "16":
            ...
            //Este no lo he entendido... todas las ciudades??
            db.collection('companies').aggregate([{ $match: { "offices.city": "London" } }, { $unwind: "$offices" }, { $project: { name: 1, _id: 0, "offices.city": 1 } }]).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "17":
            ...
            // si se refiere a que tambien es valido networking, hay que quitar el $
            db.collection('companies').find({ tag_list: /social-network$/i, founded_year: { $gte: 2002 }, founded_year: { $lte: 2016 }, "offices.city": "New York" }, { name: 1, _id: 0 }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
