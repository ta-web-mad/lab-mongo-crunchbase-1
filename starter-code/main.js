      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "7":
            ...
            db.collection('companies').find({"number_of_employees": {$gt: 100000}} , {name: 1, _id: 0}).sort({"number_of_employees":1}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          case "9":
           ...
           case "10":
            ...
            //No sabemos como sacar los productos de FB
            db.collection('companies').find({"name":"Facebook"},{number_of_employees: 1, _id: 0}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
