      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "3":
            ...
            db.collection('companies').find({name: "Facebook"},{number_of_employees: 1, name: 1, _id: 0 }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({name: "Facebook"}, {"relationships.is_past":1, name: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            case "16": // porque no sale la direcccion?
            // 16. Names and locations of companies that have offices in London
            db.collection('companies').find({"offices.city": "London"}, {name: 1, "offices.addres1": 1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({$and: [{tag_list: "social-networking"}, {founded_year:{$gte:2002}}, {founded_year:{$lte:2016}}, {"offices.city": "New York"}]}, {name:1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
