...
mongoClient.connect(`mongodb://localhost:27017/chrunchbase`, (error, db) => {
      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "3":
            ...
            db.collection('companies').find( undefined, {name: 1,number_of_employees: 1,_id: 0}).sort({"number_of_employees": -1}).limit(10).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ name: 'Facebook' }, {"relationships.person.first_name": 1, "relationships.person.last_name": 1, _id: 0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({$and: [{"tag_list": "social-network"}, {founded_year: {$gte: 2002}}, {founded_year: {$lte: 2016}}]}, { name: 1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
